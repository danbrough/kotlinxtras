...
import org.danbrough.kotlinxtras.core.sqlite.*
...
fun MemScope.processStatement(
  db: CPointerVar<sqlite3>,
  sql: String,
  processStmt: (CPointerVar<sqlite3_stmt>) -> Unit = {}
) {
...
fun main(args: Array<String>) {
  ...
  val logMessage =
    if (args.isNotEmpty()) "\"${args[0]}\"" else "'log message at ' || CURRENT_TIMESTAMP"
  ...
  memScoped {
    ...
    runCatching {
      ...
      processStatement(dbPtr, "SELECT SQLITE_VERSION()") {
        log.info(
          "SQLITE VERSION: ${
            sqlite3_column_text(it.value, 0)?.reinterpret<ByteVar>()?.toKString()
          }"
        )
      }
